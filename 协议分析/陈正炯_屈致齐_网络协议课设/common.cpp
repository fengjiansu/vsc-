...
void printUnreachable(struct icmp_unreachable* icmp_unreachable, int len)
{
	...
	WARNING("unused:          0x%08x\n", ntohs(icmp_unreachable->icmp_unused));
...
void printEcho(struct icmp_echo* icmp_echo, int len)
{
	...
	len -= hlen;
	WARNING("id:              %d(0x%04x)\n", ntohs(icmp_echo->icmp_id),ntohs(icmp_echo->icmp_id));
	WARNING("seq:             %d(0x%04x)\n", ntohs(icmp_echo->icmp_seq),ntohs(icmp_echo->icmp_seq));
	ERR_PRINT("------------ data ------------\n");
	printData(data, len);
}
void printIcmp(struct icmp* icmp, int len) {
	...
	WARNING("checksum:         %d(0x%04x)\n", ntohs(icmp->icmp_cksum), ntohs(icmp->icmp_cksum));
	
	hlen = sizeof(struct icmp);
	if(icmp->icmp_type==0||icmp->icmp_type==8)
	{
		printEcho((struct icmp_echo*)icmp , len);
	}
	else if(icmp->icmp_type==3)
	{
		printUnreachable((struct icmp_unreachable*)icmp,len);
	}
